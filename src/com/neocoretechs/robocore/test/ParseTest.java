...
public class ParseTest {
	...
	public static void parseLinks(String urlc) throws IOException {
		Document doc = Jsoup.connect(urlc)
				.userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36")
				.get();
		Elements links = doc.select("a[href]");
	...
	public static void extractContentUrl() throws Exception {
		while (!stack.isEmpty()) {
	...
	public static void extractStructureUrl() throws Exception {
		while (!stack.isEmpty()) {
		    String url = stack.pop();
			System.out.println(">>>parsing url from link:"+url);
		    String s = ParseTest.parseUrl(url,"//a");
			if(s != null)
				System.out.println(s);
			else
	...
	public static void main(String[] args) throws Exception {
		if(args[0].startsWith("http")) {
			String f = args[0];
			String s = ParseTest.parseUrl(f,"//a");
			if(s != null)
				System.out.println(s);
			else
				System.out.println("no text for "+f);
