...
public class controllerStopped implements Runnable {
	...
	public controllerStopped(AsynchDemuxer asynchDemuxer, Map<String, TopicList> topics) {
		...
		//
		this.topicList = new TopicList(asynchDemuxer, topicNames.CONTROLLERSTOPPED.val(), 8) {
			@Override
		...
		topics.put(topicNames.CONTROLLERSTOPPED.val(), topicList);
	}
	@Override
	public void run() {
		while(shouldRun) {
			synchronized(mutex) {
				try {
					...
					while(directive != null && 
						 !(directive.equals(topicNames.CONTROLLERSTOPPED.val()) && asynchDemuxer.isLineTerminal(data)) ) {
