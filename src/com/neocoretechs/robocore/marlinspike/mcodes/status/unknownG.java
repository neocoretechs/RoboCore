...
public class unknownG implements Runnable {
	...
	public unknownG(AsynchDemuxer asynchDemuxer, Map<String, TopicList> topics) {
		...
		// UNNOWNG error
		//
		this.topicList = new TopicList(asynchDemuxer, topicNames.UNKNOWNG.val(), 2) {
			@Override
		...
		topics.put(topicNames.UNKNOWNG.val(), topicList);
	}
	@Override
	public void run() {
		while(shouldRun) {
			synchronized(mutex) {
				try {
					...
					while(directive != null && 
						 !(directive.equals(topicNames.UNKNOWNG.val()) && asynchDemuxer.isLineTerminal(data)) ) {
