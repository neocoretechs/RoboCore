...
public class noMorG implements Runnable {
	...
	public noMorG(AsynchDemuxer asynchDemuxer, Map<String, TopicList> topics) {
		...
		topics.put(topicNames.NOMORGCODE.val(), topicList);
	}
	@Override
	public void run() {
		while(shouldRun) {
			synchronized(mutex) {
				try {
					...
					while(directive != null && 
						 !(directive.equals(topicNames.NOMORGCODE.val()) && asynchDemuxer.isLineTerminal(data)) ) {
							data = asynchDemuxer.getMarlinLines().takeFirst();
							if(DEBUG)
