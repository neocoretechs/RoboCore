...
public class PWMcontrolsetting implements Runnable {
	...
	public PWMcontrolsetting(AsynchDemuxer asynchDemuxer, Map<String, TopicList> topics) {
		...
		topics.put(topicNames.PWMCONTROLSETTING.val(), topicList);
	}
	@Override
	public void run() {
		while(shouldRun) {
			synchronized(mutex) {
				try {
					...
					while(directive != null && 
						 !(directive.equals(topicNames.PWMCONTROLSETTING.val()) && asynchDemuxer.isLineTerminal(data)) ) {
