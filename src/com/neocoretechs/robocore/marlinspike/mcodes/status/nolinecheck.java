...
public class nolinecheck implements Runnable {
	...
	String data;
	public nolinecheck(AsynchDemuxer asynchDemuxer, Map<String, TopicList> topics) {
		...
		topics.put(topicNames.NOLINECHECK.val(), topicList);
	}
	@Override
	public void run() {
		while(shouldRun) {
			synchronized(mutex) {
				try {
					...
					while(directive != null && 
						 !(directive.equals(topicNames.NOLINECHECK.val()) && asynchDemuxer.isLineTerminal(data)) ) {
							data = asynchDemuxer.getMarlinLines().takeFirst();
							if(DEBUG)
