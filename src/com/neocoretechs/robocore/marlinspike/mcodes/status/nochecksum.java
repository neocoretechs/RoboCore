...
public class nochecksum implements Runnable {
	...
	public nochecksum(AsynchDemuxer asynchDemuxer, Map<String, TopicList> topics) {
		...
		// NOCHECKSUM error
		//
		this.topicList = new TopicList(asynchDemuxer, topicNames.NOCHECKSUM.val(), 2) {
			@Override
		...
		topics.put(topicNames.NOCHECKSUM.val(), topicList);
	}
	@Override
	public void run() {
		while(shouldRun) {
			synchronized(mutex) {
				try {
					...
					while(directive != null && 
						 !(directive.equals(topicNames.NOCHECKSUM.val()) && asynchDemuxer.isLineTerminal(data)) ) {
