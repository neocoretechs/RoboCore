...
public class AsynchDemuxer implements Runnable {
	...
	private synchronized void init() {
		...
		synchronized(mutex) {
			...
			topics.put(topicNames.M5.val(), new M5(this).getTopicList());
			//
			// M6
			//
			//if(DEBUG)
			...
			topics.put(topicNames.M13.val(), new M13(this).getTopicList());
			//
			// M14
			//
			//if(DEBUG)
	 ...
	 */
	public static boolean isLineTerminal(String line) {
		int endDelim;
		String fop;
		try {
			if((line.charAt(0)) == '<' ) {
	 ...
	 */
	public static String extractPayload(String line, String directive) {
		int endDelim = -1;
		String fop;
		try {
			if(line.charAt(0) == '<' ) {
